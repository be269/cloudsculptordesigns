...
interface ModelProps {
  ...
  rotationX?: number;
  rotationY?: number;
  modelScale?: number; // Override scale factor for this piece
}

function Model({ url, color, color2, color3, isMetallic, position = [0, 0, 0], rotationX = 0, rotationY = 0, modelScale = 1.0 }: ModelProps) {
  ...
  const { geometry, baseScale, yMin, yMax } = useMemo(() => {
    ...
    // Apply additional X rotation if specified (in degrees)
    if (rotationX !== 0) {
      cloned.rotateX((rotationX * Math.PI) / 180);
    }
  ...
  }, [rawGeometry, rotationX, rotationY]);
...
interface CompositeAlien2SceneProps {
  bodyUrl: string;
  headUrl: string;
  leftArmUrl: string;
  rightArmUrl: string;
...
interface STLViewerCompositeAlien2Props {
  className?: string;
  colorIndex: number;
  sizeIndex: number;
...
export default function STLViewerCompositeAlien2({
  className = "",
  colorIndex,
  sizeIndex,
  onColorChange
}: STLViewerCompositeAlien2Props) {
    ...
    <div className={`relative ${className}`} style={{ minHeight: "100%", height: "100%" }}>
      ...
      >
        ...
        <CompositeAlien2Scene
          ...
          alienColor2={currentColor.hex2}
          alienColor3={currentColor.hex3}

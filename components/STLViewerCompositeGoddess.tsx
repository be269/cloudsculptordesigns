...
function Model({ url, color, color2, color3, isMetallic, position = [0, 0, 0], rotationY = 0 }: ModelProps) {
...
interface CompositeGoddessSceneProps {
  baseUrl: string;
  planterUrl: string;
  planterColor: string;
  planterColor2?: string;
  planterColor3?: string;
  isRotating: boolean;
  scale: number;
  isMetallic: boolean;
}

function CompositeGoddessScene({
  baseUrl,
  planterUrl,
  planterColor,
  planterColor2,
  ...
  return (
    <group ref={groupRef} scale={[scale, scale, scale]}>
      {/* Base/Tray - at bottom, same color as planter */}
      <Model
        url={baseUrl}
        color={planterColor}
        color2={planterColor2}
        color3={planterColor3}
        isMetallic={isMetallic}
        position={[0, -0.6, 0]}
      />
      {/* Forest Goddess Planter - on top of base */}
      <Model
        url={planterUrl}
        color={planterColor}
        color2={planterColor2}
...
interface STLViewerCompositeGoddessProps {
  className?: string;
  colorIndex: number;
  sizeIndex: number;
...
export default function STLViewerCompositeGoddess({
  className = "",
  colorIndex,
  sizeIndex,
  onColorChange
}: STLViewerCompositeGoddessProps) {

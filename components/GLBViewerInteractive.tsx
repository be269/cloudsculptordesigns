...
function Model({ url, color, isRotating, scale, onLoaded }: ModelProps) {
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        // Find and store reference to the body material
        loadedModel.traverse((child) => {
          if (child instanceof THREE.Mesh) {
            if (Array.isArray(child.material)) {
              child.material.forEach((mat) => {
                if (mat.name === "Body") {
                  bodyMaterialRef.current = mat as THREE.MeshStandardMaterial;
                }
              });
            } else if (child.material.name === "Body") {
              bodyMaterialRef.current = child.material as THREE.MeshStandardMaterial;
            }
          }
        });
  ...
  // Update body color when color prop changes
  useEffect(() => {
...
interface GLBViewerInteractiveProps {
  modelUrl: string;
  className?: string;
}

export default function GLBViewerInteractive({ modelUrl, className = "" }: GLBViewerInteractiveProps) {
